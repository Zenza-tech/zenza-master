...
One command, installs dependencies for whichever apps need them (only
`03-zenza-fid-app` has any — `02-zenza-tech-site` deliberately uses zero
npm packages, just Node's built-ins). This does **not** ship `node_modules`
inside the zip — one of its dependencies (`better-sqlite3`) compiles
platform-specific binaries, so a `node_modules` folder built on one OS
would silently fail on another. Running `npm install` (which `setup.js`
does for you) always fetches the correct build for whatever machine runs
it — that's the only version of "included" that actually works on every
developer's laptop, not just the one it was zipped on.

## QA & Testing

`QA-TESTING-REPORT.md` at the root of this package documents a full
functional and security audit of the Zenza FID app — what was tested, real
bugs that were found and fixed, and an honest list of what hasn't been
tested yet (no automated test suite, no load testing, no third-party
penetration test). Read that before treating this as production-hardened.

## Multi-tenant, as of this version

Zenza FID now serves multiple customer organizations from one
deployment. Each organization's fraud data lives in its own separate
database — isolation is architectural, and was verified with adversarial
cross-tenant testing (QA report §11). Two features sit on top of that:

- **A cross-institution fraud network** (opt-in) that lets participating
  organizations corroborate a suspect against each other's intelligence
  using keyed hashes — never raw identifiers, names, or case notes.
- **A platform administration console** for onboarding, suspending,
  archiving and permanently purging customer organizations, deliberately
  limited to operational metadata rather than customer case content.

`03-zenza-fid-app/COMPLIANCE-NDPA.md` documents how personal data is
handled, and is written to be handed to a Nigerian data-protection lawyer
for review. It is explicit about what would currently fail a customer's
due diligence — worth reading before a sales conversation.

## Extending this codebase

`03-zenza-fid-app/ENGINEERING.md` is written for the engineers picking
this up — the pattern every route follows, how to add a new detection
rule type, and specifically how to build the core-banking API integration
layer (the next major piece, deliberately not built against a guess — the
connector interface and a working mock connector already exist at
`03-zenza-fid-app/server/services/coreBanking/` as the starting point).
