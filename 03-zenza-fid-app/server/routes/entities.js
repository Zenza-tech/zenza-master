...
const { validateIdentifiers } = require("../utils/validators");
const { indexEntity } = require("../services/search");
...
router.get("/", requirePermission("entities.view"), (req, res) => {
  ...
  const withIdentifiers = rows.map((r) => ({ ...r, identifiers: getIdentifiers(db, r.id) }));
...
router.get("/:id", requirePermission("entities.view"), (req, res) => {
  const db = req.tenantDb;
  const entity = getEntitySnapshot(db, req.params.id);
...
router.post("/", requirePermission("entities.create"), (req, res) => {
  ...
  const formatError = validateIdentifiers(identifiers);
  if (formatError) return res.status(400).json({ ok: false, error: formatError });

  if (!force) {
    const dupes = findDuplicates(db, identifiers);
    if (dupes.length > 0) {
  ...
  const txn = db.transaction(() => {
    ...
    const snapshot = getEntitySnapshot(db, entityId);
    db.prepare(
  ...
  const entityId = txn();
  indexEntity(db, db.prepare("SELECT * FROM entities WHERE id = ?").get(entityId));
  recordInput(db, req.user.id, "entity_full_name", full_name);
  if (risk_notes) recordInput(db, req.user.id, "entity_risk_notes", risk_notes);
  res.status(201).json({ ok: true, entity: getEntitySnapshot(db, entityId) });
});
...
router.put("/:id", requirePermission("entities.edit"), (req, res) => {
  ...
  const txn = db.transaction(() => {
    ...
    const snapshot = getEntitySnapshot(db, existing.id);
    db.prepare(
      `INSERT INTO entity_versions (entity_id, changed_by, change_summary, snapshot, created_at)
       VALUES (?, ?, ?, ?, ?)`
  ...
  txn();
  indexEntity(db, db.prepare("SELECT * FROM entities WHERE id = ?").get(existing.id));
  res.json({ ok: true, entity: getEntitySnapshot(db, existing.id) });
});
...
router.post("/bulk", requirePermission("entities.create"), (req, res) => {
  ...
  records.forEach((rec, idx) => {
    try {
      ...
      const formatError = validateIdentifiers(identifiers);
      if (formatError) {
        results.errors.push({ row: idx, error: formatError });
        return;
      }
      const dupes = findDuplicates(db, identifiers);
      if (dupes.length > 0) {
